...
@property (nonatomic,assign)CGFloat  firstEndPoint;
@property (nonatomic,assign)CGFloat  twoEndPoint;
@property (nonatomic,assign)CGFloat  lastEndPoint;
...
- (void)animation {
    switch (self.branchTexts.count) {
        case 1:{
            switch (self.tagAnimationStyle) {
                case XHTagAnimationStyleAllLeft: {
                    [self setupBranchLayer:self.midBranchLayer branchPoint:self.branchPoints[3]];
                    break;
                }
                case XHTagAnimationStyleAllRight: {
                    [self setupBranchLayer:self.midBranchLayer branchPoint:self.branchPoints[0]];
                    break;
                }
    ...
    [self.topBranchLayer animationDelay:0.0 completion:^(BOOL finished, XHBranchLayer *branchLayer) {
        if (finished) {
            [weakSelf.topBranchTextView showInPoint:branchLayer.midPoint direction:branchLayer.direction];
    ...
    [self.midBranchLayer animationDelay:0.0 completion:^(BOOL finished, XHBranchLayer *branchLayer) {
        if (finished) {
            [weakSelf.midBranchTextView showInPoint:branchLayer.midPoint direction:branchLayer.direction];
        }
    }];
    [self.bottomBranchLayer animationDelay:0.0 completion:^(BOOL finished, XHBranchLayer *branchLayer) {
        if (finished) {
            [weakSelf.bottomBranchTextView showInPoint:branchLayer.midPoint direction:branchLayer.direction];
        }
    }];
}
...
- (void)handlePanGestureRecognizer:(UIPanGestureRecognizer *)panGestureRecognizer {
    ...
    if (_currentPoint.x >self.frame.size.width/2 &&_currentPoint.x < _imageSize.width -self.frame.size.width/2) {
        self.center = CGPointMake(self.center.x + point.x, self.center.y);
    }
    if (_currentPoint.y >self.frame.size.width/2 &&_currentPoint.y < _imageSize.height -self.frame.size.height/2){
    ...
    _currentPoint = CGPointMake(self.center.x + point.x, self.center.y + point.y);
...
- (NSMutableArray *)branchPoints {
    if (!_branchPoints) {
        ...
        switch (self.branchTexts.count) {
            case 1:{
                _lastEndPoint = [self textCalculatedSizeWidth: [_branchTexts lastObject]].width + 40;
                break;
            }
            case 2:{
                _firstEndPoint = [self textCalculatedSizeWidth: [_branchTexts firstObject]].width + 40;
                _lastEndPoint = [self textCalculatedSizeWidth: [_branchTexts lastObject]].width + 40;
                break;
            }
            case 3:{
                _firstEndPoint = [self textCalculatedSizeWidth: [_branchTexts firstObject]].width + 40;
                _twoEndPoint = [self textCalculatedSizeWidth: [_branchTexts objectAtIndex:1]].width + 30;
                _lastEndPoint = [self textCalculatedSizeWidth: [_branchTexts lastObject]].width + 40;
                break;
            }
        ...
        [_branchPoints addObject:[XHBranchPoint initlizerStartPoint:center
                                                           ...
                                                           endPoint:CGPointMake(center.x - _lastEndPoint, center.y + 35)
                                                          direction:XHBranchLayerDirectionLeft]];
        [_branchPoints addObject:[XHBranchPoint initlizerStartPoint:center
                                                           midPoint:CGPointMake(center.x - 10, center.y)
                                                           endPoint:CGPointMake(center.x - _twoEndPoint, center.y)
                                                          direction:XHBranchLayerDirectionLeft]];
        [_branchPoints addObject:[XHBranchPoint initlizerStartPoint:center
                                                           midPoint:CGPointMake(center.x - 20, center.y - 35)
                                                           endPoint:CGPointMake(center.x - _firstEndPoint, center.y - 35)
                                                          direction:XHBranchLayerDirectionLeft]];
...
- (CGSize)textCalculatedSizeWidth:(NSString *)text{
